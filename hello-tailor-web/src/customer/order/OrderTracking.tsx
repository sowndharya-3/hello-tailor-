...
import { itemTitle } from '@/lib/bookingItems';
  ...
  return (
    <div>
      ...
      <div className="p-4 pb-8 sm:px-6">
        ...
        {booking.items && booking.items.length > 0 && <div className="mb-5"><p className="mb-2 font-semibold text-ht-text">Garments ({booking.items.length})</p><BookingItemsList items={booking.items} /></div>}
        ...
        {booking.quoteStatus && booking.quoteStatus !== 'Pending' && booking.quoteStatus !== 'Rejected' && <div className="mb-5 rounded-ht-card border border-ht-border bg-white p-4"><p className="mb-3 font-semibold text-ht-text">Tailor Quotation</p>{booking.items?.length ? booking.items.map((item, index) => <div key={item.id} className="mb-2 rounded-ht-input bg-ht-bg p-3"><p className="text-sm font-semibold text-ht-text">Item {index + 1} – {itemTitle(item)}{item.quantity > 1 ? ` × ${item.quantity}` : ''}</p><div className="mt-1 flex justify-between text-sm"><span className="text-ht-text-secondary">Stitching</span><span className="text-ht-text">₹{item.stitchingCharge ?? 0}</span></div><div className="flex justify-between text-sm"><span className="text-ht-text-secondary">Material</span><span className="text-ht-text">₹{item.materialCost ?? 0}</span></div></div>) : null}{[
          ...(booking.items?.length ? [] : [['Stitching', booking.stitchingCharge], ['Material', booking.materialCost]]), ['Pickup', booking.pickupFee], ['Delivery', booking.deliveryFee], ['Customization', booking.customizationCharge], ['Tax', booking.tax], ['Discount', booking.discount ? -booking.discount : 0],
