// Only these five customer routes appear in the tab bar. Nested routes stay
// reachable through router.push(), including routes added in future updates.
...
const TAB_ROUTES = new Set(['index', 'tailors', 'bookings', 'messages/index', 'profile']);
  ...
  return (
    <Tabs
      screenOptions={({ route }) => ({
        ...
        tabBarItemStyle: TAB_ROUTES.has(route.name) ? undefined : { display: 'none' },
        tabBarButton: TAB_ROUTES.has(route.name) ? undefined : () => null,
      })}
    >
