...
// screen. Ported from tailor-app/components/OrderDetailShared.tsx, adapted to the shared
// `Booking` type (advancePaid/balancePaid are booleans here, not 'Paid'/'Pending' strings).
...
export function CustomerInfoCard({ order }: { order: Booking }) {
  return (
    <Card>
      ...
      <InfoLine icon={order.pickupType === 'Home Delivery' || order.pickupType === 'Tailor Pickup' ? 'home-outline' : 'storefront-outline'} label={order.pickupType} />
    </Card>
  );
}
  ...
  return (
    <Card>
      ...
      <View style={styles.finRow}>
        ...
        <StatusPill label={order.advancePaid ? 'Paid' : 'Pending'} tone={order.advancePaid ? 'success' : 'warning'} />
      </View>
      <View style={styles.finRow}>
        <View>
        ...
        <StatusPill label={order.balancePaid ? 'Paid' : 'Pending'} tone={order.balancePaid ? 'success' : 'warning'} />
      </View>
    </Card>
  );
}
