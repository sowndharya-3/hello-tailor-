...
export default function Commission() {
  const { show } = useToast();
  const commissionEntries = useStore((s) => s.commissions);
  ...
  // ponytail: store models a single flat commissionRate, not a separate premium-tier rate —
  // keep the premium field local-only until the store grows a per-tier rate.
  ...
  ), [commissionEntries, tailorFilter, period]);
  ...
  const columns: Column<CommissionEntry>[] = [
    ...
    { key: 'orderAmount', header: 'Order Amount', sortValue: (c) => c.orderAmount, render: (c) => formatMoney(c.orderAmount) },
    { key: 'commissionRate', header: 'Rate', render: (c) => `${c.commissionRate}%` },
    { key: 'commissionAmount', header: 'Commission', sortValue: (c) => c.commissionAmount, render: (c) => formatMoney(c.commissionAmount) },
    ...
    <div>
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-3 mb-5">
        <Card className="xl:col-span-1">
          <h3 className="mb-4 text-base font-semibold text-ht-navy">Commission Rate Settings</h3>
          <div className="flex flex-col gap-4">
            ...
            <Button icon={<Save size={16} />} onClick={() => { setCommissionRate(Number(rate) || 0); show('success', 'Commission rates updated successfully.'); }}>Save Settings</Button>
          </div>
        </Card>
        <Card className="xl:col-span-2">
          <h3 className="mb-4 text-base font-semibold text-ht-navy">Commission Collected</h3>
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-xl bg-ht-info-bg p-4"><div className="text-sm text-ht-ocean font-medium">Total Collected</div><div className="mt-1 text-2xl font-bold text-ht-navy">{formatMoney(totalCollected)}</div></div>
            <div className="rounded-xl bg-[#FFFAEB] p-4"><div className="text-sm text-ht-warning font-medium">Pending Collection</div><div className="mt-1 text-2xl font-bold text-ht-navy">{formatMoney(totalPending)}</div></div>
          </div>
        </Card>
      </div>
