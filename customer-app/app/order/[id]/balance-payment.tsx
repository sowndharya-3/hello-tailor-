...
export default function BalancePayment() {
  ...
  if (state === 'success') {
    return (
      <View style={styles.centerWrap}>
        ...
        <Text style={styles.successSub}>₹{order.balanceDue} balance paid for order {order.id}</Text>
        <Button label="View Invoice" style={{ marginTop: spacing.lg, width: '100%' }} onPress={() => router.replace(`/order/${order.id}/invoice`)} />
      </View>
    );
  }
  ...
  return (
    <View style={styles.wrap}>
      <ScreenHeader title="Pay Balance" subtitle={order.id} />
      <View style={{ padding: spacing.screenH }}>
        <View style={styles.card}>
          <Row label="Total Order Value" value={`₹${order.amount}`} />
          <Row label="Advance Paid" value={`-₹${order.advancePaid}`} muted />
          <Row label="Discount" value={order.discount ? `-₹${order.discount}` : '₹0'} muted />
          <Row label="Tax (GST)" value={`₹${order.tax}`} muted />
          <View style={styles.divider} />
          <Row label="Balance Due" value={`₹${order.balanceDue}`} bold />
        </View>
        <Button label={`Pay ₹${order.balanceDue}`} onPress={pay} style={{ marginTop: spacing.lg }} />
      </View>
    </View>
  );
}
