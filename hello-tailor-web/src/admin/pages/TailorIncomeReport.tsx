...
export default function TailorIncomeReport() {
  ...
  const rows: Row[] = useMemo(() => tailors.map((t) => ({
    ...t,
    grossEarnings: t.income,
    netEarnings: t.income - t.commissionPaid,
    orders: bookings.filter((b) => b.tailorId === t.id).length,
  })), [tailors, bookings]);
  ...
  const columns: Column<Row>[] = [
    ...
    { key: 'grossEarnings', header: 'Gross Earnings', sortValue: (r) => r.grossEarnings, render: (r) => formatMoney(r.grossEarnings) },
    { key: 'commissionPaid', header: 'Commission', sortValue: (r) => r.commissionPaid, render: (r) => formatMoney(r.commissionPaid) },
    { key: 'netEarnings', header: 'Net Earnings', sortValue: (r) => r.netEarnings, render: (r) => <span className="font-semibold text-ht-navy">{formatMoney(r.netEarnings)}</span> },
  ];
  ...
  return (
    <div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Card><div className="text-sm text-ht-text-secondary">Total Gross Earnings</div><div className="mt-1 text-2xl font-bold text-ht-navy">{formatMoney(totalGross)}</div></Card>
        <Card><div className="text-sm text-ht-text-secondary">Total Net Earnings (After Commission)</div><div className="mt-1 text-2xl font-bold text-ht-navy">{formatMoney(totalNet)}</div></Card>
      </div>
