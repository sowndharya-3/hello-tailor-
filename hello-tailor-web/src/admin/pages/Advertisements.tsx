...
const blankAd: Advertisement = {
  ...
  startDate: '2026-08-21', endDate: '2026-09-21', status: 'Scheduled',
};
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <Card key={ad.id} padded={false} className="overflow-hidden">
            ...
            <div className="p-4">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <div className="text-sm font-semibold text-ht-navy">{ad.title}</div>
                  <div className="text-xs text-ht-text-secondary">{ad.type} · {ad.placement}</div>
                </div>
                <StatusBadge status={ad.status} />
              </div>
              <div className="mt-2 text-xs text-ht-text-secondary">{ad.startDate} → {ad.endDate}</div>
        ...
        {ads.length === 0 && <p className="text-sm text-ht-text-secondary">No advertisements yet.</p>}
      </div>
      ...
      <ConfirmDialog
        ...
        confirmLabel="Delete"
        destructive
        onConfirm={() => { if (deleting) { deleteAd(deleting.id); show('success', 'Advertisement deleted.'); } setDeleting(null); }}
