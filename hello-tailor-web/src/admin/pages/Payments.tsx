...
export default function Payments() {
  ...
  const filtered = useMemo(() => payments.filter((p) =>
    (!search || p.bookingId.toLowerCase().includes(search.toLowerCase()) || p.customerName.toLowerCase().includes(search.toLowerCase()) || p.gatewayRef.toLowerCase().includes(search.toLowerCase())) &&
    (!type || p.type === type) &&
    (!status || p.status === status)
  ), [payments, search, type, status]);
  ...
  return (
    <div>
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Card><div className="text-sm text-ht-text-secondary">Total Collected</div><div className="mt-1 text-2xl font-bold text-ht-navy">{formatMoney(totalCollected)}</div></Card>
        <Card><div className="text-sm text-ht-text-secondary">Pending Payments</div><div className="mt-1 text-2xl font-bold text-ht-warning">{formatMoney(totalPending)}</div></Card>
        <Card><div className="text-sm text-ht-text-secondary">Total Refunded</div><div className="mt-1 text-2xl font-bold text-ht-error">{formatMoney(totalRefunded)}</div></Card>
      </div>
