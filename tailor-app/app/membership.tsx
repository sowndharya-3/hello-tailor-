...
export default function Membership() {
  ...
  return (
    <View style={styles.container}>
      <ScreenHeader title="Tailor Membership" onBack={() => router.back()} />
      <ScrollView contentContainerStyle={styles.scroll}>
        {profile.activePlan && (
          <Card style={styles.activeCard}>
            <View style={styles.row}>
              <Ionicons name="diamond" size={20} color={colors.gold} />
              <Text style={styles.activeLabel}>Active Plan: {profile.activePlan.name}</Text>
            </View>
            <Text style={styles.activeExpiry}>
              Expires on {profile.planExpiry ? new Date(profile.planExpiry).toLocaleDateString('en-IN') : '—'}
            </Text>
          </Card>
        )}

        {membershipPlans.map((plan) => {
          const isActive = profile.activePlan?.id === plan.id;
          return (
...
const styles = StyleSheet.create({
  ...
  activeExpiry: { fontFamily: font.regular, fontSize: 12, color: colors.textSecondary, marginTop: 4, marginLeft: 28 },
