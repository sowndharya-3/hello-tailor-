// Ported from tailor-app/app/(tabs)/income.tsx — driven by useMyBookings() (status 'Delivered'
// is the shared store's completed stage). Weekly/monthly breakdown chart data stays mocked
// (matches source app), only the top-line totals are computed from real booking amounts.
        ...
        {completed.slice(0, 6).map((b) => (
          <Card key={b.id} style={{ marginBottom: spacing.sm }}>
            <View style={styles.payoutRow}>
              <View>
                <Text style={styles.payoutName}>{b.customerName}</Text>
                <Text style={styles.payoutSub}>{b.category} · {b.id}</Text>
              </View>
              <Text style={styles.payoutAmount}>+₹{b.amount.toLocaleString('en-IN')}</Text>
            </View>
          </Card>
        ))}
      </ScrollView>
