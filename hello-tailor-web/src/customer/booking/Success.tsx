...
export default function BookingSuccess() {
  ...
  return (
    <div className="flex min-h-[70vh] flex-col items-center justify-center px-8 text-center">
      ...
      <h1 className="text-[22px] font-semibold text-ht-text">Booking Confirmed!</h1>
      <p className="mt-2 text-[14px] text-ht-text-secondary">Your order has been placed successfully. We'll notify you as soon as the tailor accepts it.</p>

      <div className="mt-6 w-full max-w-sm rounded-ht-card border border-ht-border bg-ht-card p-5 text-center">
        <p className="text-[12px] text-ht-text-secondary">Order ID</p>
        <p className="mt-1 text-[16px] font-semibold text-ht-navy">{id ?? '—'}</p>
      </div>

      <div className="mt-5 w-full max-w-sm">
        <Button label="Track Order" onClick={() => navigate(id ? `/customer/order/${id}` : '/customer/bookings', { replace: true })} />
      </div>
