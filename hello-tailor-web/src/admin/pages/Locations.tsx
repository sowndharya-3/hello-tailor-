...
export default function Locations() {
  ...
  function toggle(loc: LocationEntry) {
    toggleLocationServiceable(loc.id);
  ...
  function addLocationEntry() {
    ...
    addLocation({ id: `LOC-${locations.length}`, state: form.state, city: form.city, serviceable: true, tailorCount: 0 });
  ...
  const columns: Column<LocationEntry>[] = [
    ...
    { key: 'tailorCount', header: 'Tailors', sortValue: (l) => l.tailorCount },
    {
      ...
      render: (l) => (
        <button onClick={() => toggle(l)} className={`inline-flex h-6 w-11 items-center rounded-full transition-colors ${l.serviceable ? 'bg-ht-success' : 'bg-ht-disabled-bg'}`}>
  ...
  return (
    <div>
      ...
      <Modal open={creating} onClose={() => setCreating(false)} title="Add Location" footer={<><Button variant="secondary" onClick={() => setCreating(false)}>Cancel</Button><Button onClick={addLocationEntry}>Add</Button></>}>
